...
export class ListarImagenComponent implements OnInit {
  ...
  constructor(private imagenService: ImagenService) { 
    this.suscription = this.imagenService.getTerminoBusqueda().subscribe(data => {
      this.termino = data;
      this.paginaActual = 1;
      this.loading = true;
  ...
  obtenerImagenes() {
    this.imagenService.getImagenes(this.termino, this.imagensPorPagina, this.paginaActual).subscribe(data => {
      this.loading = false;
      this.imagenService.verListado = true;
      
      if(data.hits.length === 0){
        this.imagenService.verListado = false;
      ...
      this.imagenService.setError('Opss.. ha ocurrido un error.');
      this.loading = false;
    })
  }
